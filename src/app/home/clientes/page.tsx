...
import { 
  Loader2, Search, User, Phone, ShoppingBag, X, Calendar, 
  AlertCircle, CheckCircle2, Filter, ChevronDown, ChevronLeft, 
  ChevronRight, Edit2, Check, Ban
} from "lucide-react";
...
const handleUpdatePhone = async () => {
  ...
  if (sortBy === "gasto") clientesFiltrados.sort((a, b) => b.totalGastadoUSD - a.totalGastadoUSD);
  else if (sortBy === "frecuentes") clientesFiltrados.sort((a, b) => b.cantidadPedidos - a.cantidadPedidos);
  else clientesFiltrados.sort((a, b) => {
    if (a.pedidos.length && b.pedidos.length) return new Date(b.pedidos[0].createdAt).getTime() - new Date(a.pedidos[0].createdAt).getTime();
    return 0;
  });
      ...
      <div className="flex-1 flex flex-col h-[calc(100vh-80px)] overflow-y-auto p-6 lg:p-10 pb-20">
        ...
        <div className="max-w-6xl mx-auto w-full flex flex-col md:flex-row justify-between items-center md:items-end mb-10 gap-6 z-10 relative">
          <div className="flex flex-col items-center lg:items-start space-y-2 w-full md:w-auto text-center md:text-left">
            ...
            <p className="text-[#294C29]/60 font-bold text-sm mt-3 flex items-center justify-center lg:justify-start gap-2 pt-2 uppercase tracking-widest">
            ...
            <div className="w-full sm:w-52 relative">
              ...
              <div className={`absolute top-full left-0 right-0 mt-2 bg-white border border-[#294C29]/10 rounded-2xl shadow-xl overflow-hidden z-20 transition-all duration-200 origin-top ${isFilterOpen ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}>
                {filterOptions.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => { setSortBy(opt.value as any); setIsFilterOpen(false); }}
                    className={`w-full text-left px-5 py-3.5 text-[11px] font-black uppercase tracking-widest transition-colors ${sortBy === opt.value ? "bg-[#B43E17]/10 text-[#B43E17]" : "text-[#294C29]/60 hover:bg-[#FDF8F1] hover:text-[#294C29]"}`}
                  >
                    {opt.label}
                  </button>
                ))}
            ...
            <>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-4 mt-12 mb-4">
                  <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-3 bg-white border border-[#294C29]/10 text-[#294C29] rounded-2xl hover:bg-[#FDF8F1] transition-colors disabled:opacity-50 shadow-sm">
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <div className="bg-white border border-[#294C29]/10 px-6 py-3 rounded-2xl shadow-sm">
                    <span className="text-xs font-black text-[#294C29] uppercase tracking-widest">Página <span className="text-[#B43E17] mx-1">{currentPage}</span> de {totalPages}</span>
                  </div>
                  <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="p-3 bg-white border border-[#294C29]/10 text-[#294C29] rounded-2xl hover:bg-[#FDF8F1] transition-colors disabled:opacity-50 shadow-sm">
      ...
      <div className={`fixed inset-y-0 right-0 z-50 w-full md:w-112.5 bg-white border-l border-[#294C29]/10 shadow-[text-xl_0_40px_rgba(0,0,0,0.1)] transform transition-transform duration-300 ease-in-out flex flex-col ${selectedCliente ? "translate-x-0" : "translate-x-full"}`}>
        {selectedCliente && (
          <>
            ...
            <div className="p-6 md:p-8 bg-white border-b border-[#294C29]/10 flex justify-between items-center">
              <div>
                    ...
                    <div key={pedido.id} className="bg-white p-5 rounded-2xl border border-[#294C29]/10 shadow-sm flex flex-col hover:border-[#294C29]/20 transition-colors">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        {pedido.estadoPago === "PAGADO" ? (
                          <span className="bg-[#294C29]/10 text-[#294C29] px-2.5 py-1 rounded-lg uppercase tracking-widest text-[9px] font-black flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Pagado</span>
                        ) : (
                          <span className="bg-[#B43E17]/10 text-[#B43E17] px-2.5 py-1 rounded-lg uppercase tracking-widest text-[9px] font-black flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Pendiente</span>
                        )}
                      </div>
                        ...
                        <ul className="space-y-1.5">
                          {pedido.detalles?.map(detalle => (
                            <li key={detalle.id} className="text-[13px] font-bold text-[#294C29]/80 flex justify-between leading-tight">
                              <span><span className="text-[#B43E17] font-black mr-1">{detalle.cantidad}x</span>{detalle.producto?.nombre || "Producto"}{detalle.subDetalles?.length > 0 && <span className="text-[#294C29]/40 text-[10px] uppercase ml-1">(+ Extras)</span>}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
