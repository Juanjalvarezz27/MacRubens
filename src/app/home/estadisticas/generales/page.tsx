...
interface ProduccionData { nombre: string; cantidad: number; }
...
interface GeneralStats {
  ...
  metodos: MetodoData[];
  produccionBases: ProduccionData[];
...
export default function EstadisticasGeneralesPage() {
  ...
  const handleCustomSearch = () => {
    ...
    if (new Date(startDate) > new Date(endDate)) return toast.warning("La fecha de inicio no puede ser mayor a la final");
      ...
      <div className="max-w-6xl mx-auto space-y-6 z-10 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="bg-[#294C29] p-5 sm:p-6 rounded-4xl shadow-lg relative overflow-hidden text-[#F6E4C9]">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <span className="text-xs font-black uppercase tracking-widest text-[#F6E4C9]/60">Ingresos Reales</span>
              <DollarSign className="w-5 h-5 sm:w-6 sm:h-6 text-[#EADDCA]" />
            </div>
            <div className="relative z-10">
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-bold">$</span>
                <span className="text-4xl font-black tracking-tighter leading-none">{stats?.totalUSD.toFixed(2)}</span>
              </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-4xl shadow-sm border border-[#294C29]/10">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-[#FDF8F1] flex items-center justify-center text-[#B43E17]">
                <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
            </div>
            <div>
              <span className="text-4xl font-black text-[#294C29] tracking-tighter leading-none">{stats?.totalPedidosPagados}</span>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-4xl shadow-sm border border-red-500/20">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-red-50 flex items-center justify-center text-red-500">
            ...
            <div>
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-4xl font-black text-red-500 tracking-tighter leading-none">{stats?.montoPendienteUSD.toFixed(2)}</span>
              </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-4xl shadow-sm border border-[#294C29]/10">
            ...
            <div>
              <span className="text-4xl font-black text-[#294C29] tracking-tighter leading-none">{stats?.totalClientes}</span>
              <span className="block text-xs font-bold text-[#294C29]/40 mt-2 uppercase tracking-widest">En el lapso</span>
            </div>
          </div>
        </div>

        {/* CUADRÍCULA DE PRODUCCIÓN (3 Columnas para que respiren los nombres) */}
        {stats && stats.produccionBases.length > 0 && (
          <div className="bg-white rounded-4xl p-6 lg:p-8 border border-[#294C29]/10 shadow-sm mt-6">
            <h3 className="font-black text-[#294C29] uppercase tracking-tighter text-lg flex items-center gap-2 mb-6 pl-1">
              <Pizza className="w-5 h-5 text-[#B43E17]" /> Producción Acumulada
            </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mt-6">
          <div className="bg-white rounded-4xl p-5 sm:p-6 lg:p-8 border border-[#294C29]/10 shadow-sm">
            ...
            {stats?.metodos.length === 0 ? (
              <p className="text-sm font-bold text-[#294C29]/40 text-center py-10">Sin pagos.</p>
            ) : (
              <div className="space-y-3 sm:space-y-4">
                {stats?.metodos.map((metodo, idx) => (
          ...
          <div className="bg-[#B43E17] rounded-4xl p-5 sm:p-6 lg:p-8 text-[#F6E4C9] shadow-xl relative overflow-hidden flex flex-col justify-between">
            ...
            {stats?.topProductos.length === 0 ? (
              <p className="text-sm font-bold text-[#F6E4C9]/60 text-center py-10 relative z-10">Sin ventas.</p>
            ) : (
                  ...
                  return (
                    <div key={index}>
                      <div className="flex justify-between items-end mb-2">
                        <div>
                          <span className="text-[11px] font-black uppercase tracking-widest text-[#EADDCA] mr-2">#{index + 1}</span>
                          <span className="font-black text-xs sm:text-sm uppercase tracking-widest">{prod.nombre}</span>
                        </div>
                        <div className="text-right">
                          <span className="font-black text-lg sm:text-xl leading-none block">${prod.ingresos.toFixed(2)}</span>
                          <span className="text-[10px] sm:text-[11px] font-bold text-[#EADDCA] uppercase tracking-widest">{prod.cantidad} uds</span>
                        </div>
                      </div>
                      <div className="w-full h-2.5 bg-black/20 rounded-full overflow-hidden">
                        <div className="h-full bg-white rounded-full transition-all duration-1000 ease-out relative" style={{ width: `${widthPercentage}%` }}></div>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="bg-white rounded-4xl border border-[#294C29]/10 shadow-sm mt-8 overflow-hidden transition-all">
            <button onClick={() => setIsLedgerOpen(!isLedgerOpen)} className="w-full flex flex-col md:flex-row justify-between items-start md:items-center p-5 sm:p-6 lg:p-8 bg-white hover:bg-[#FDF8F1] transition-colors focus:outline-none gap-4">
              ...
              <div className="flex items-center justify-between w-full md:w-auto gap-4">
                <span className="text-xs sm:text-sm font-bold text-[#294C29]/50 uppercase tracking-widest bg-[#FDF8F1] px-4 py-2 rounded-lg border border-[#294C29]/10">
                  {stats.historial.length} Clientes
                </span>
                <div className={`w-10 h-10 rounded-full bg-[#FDF8F1] flex items-center justify-center text-[#B43E17] transition-transform duration-300 shrink-0 ${isLedgerOpen ? 'rotate-180' : ''}`}>
              ...
              <div className="p-4 sm:p-6 lg:p-8 pt-0 border-t border-[#294C29]/5 animate-in slide-in-from-top-4 duration-300">
                      ...
                      <div key={cliente.id} className="border border-[#294C29]/10 rounded-2xl overflow-hidden bg-white shadow-sm">
                        ...
                        >
                          <div className="flex items-center gap-3 sm:gap-4 mb-3 md:mb-0">
                            <div className="w-12 h-12 rounded-xl bg-[#FDF8F1] flex items-center justify-center text-[#294C29] shrink-0">
                              ...
                              <div key={orden.id} className="bg-white p-4 sm:p-5 rounded-xl border border-[#294C29]/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                                <div className="flex-1 min-w-0">
                                  <div className="flex flex-wrap items-center gap-3 mb-2">
                                    <span className="text-[11px] sm:text-xs font-black text-[#294C29]/60 uppercase tracking-widest">
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 sm:gap-4 mt-6 sm:mt-8">
                    <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-3 sm:p-4 bg-white border border-[#294C29]/10 text-[#294C29] rounded-xl sm:rounded-2xl disabled:opacity-50">
                      <ChevronLeft className="w-5 h-5" />
                    </button>
                    <div className="bg-[#FDF8F1] border border-[#294C29]/10 px-5 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl shadow-sm">
                      <span className="text-xs sm:text-sm font-black text-[#294C29] uppercase tracking-widest">
                        Página <span className="text-[#B43E17] mx-1">{currentPage}</span> de {totalPages}
                      </span>
                    </div>
                    <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="p-3 sm:p-4 bg-white border border-[#294C29]/10 text-[#294C29] rounded-xl sm:rounded-2xl disabled:opacity-50">
